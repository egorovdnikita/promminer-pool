...
- **Стрелки — Linear**: `cd`, `cv`, `cl`, `sortUp`, `ext`, `sortv`. Они приходят обводкой,
  поэтому каждому пути обязателен `fill="none"` — обёртка `sv()` ставит
  `fill="currentColor"` на `<svg>`, и контур залился бы сплошной фигурой.
