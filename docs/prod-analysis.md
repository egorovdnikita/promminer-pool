...
> Содержимое самих файлов с макетами через Figma MCP не читается — страницы возвращаются
> пустыми (нужен файл, открытый в десктоп-приложении Figma, либо ссылка с `node-id`).
> Доступен только поиск по опубликованным библиотекам.
