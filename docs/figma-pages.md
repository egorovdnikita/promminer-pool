...
Три страницы дают один и тот же набор из 11 модалок. Телефон и почта устроены
одинаково: добавление и изменение в два шага (значение → код), отвязка — красное
подтверждение **на 500**, а не 550, и отдельный шаг с кодом. У Telegram шагов нет:
ссылка на бота и поле, шапка с иллюстрацией `modal-tg.png`.
