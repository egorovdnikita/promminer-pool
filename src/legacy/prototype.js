...
const AXES={
  ...
  numfmt:{g:'Данные',label:'Формат чисел',note:'Разделители разрядов и дробной части',
    opts:[['ru','1 000,00'],['en','1,000.00'],['plain','1000.00']]},
  tzview:{g:'Данные',label:'Время',note:'В какой зоне печатаются даты',
    opts:[['local','Местное'],['utc','UTC']]},
  ...
  wfilt:{g:'Воркеры',label:'Фильтр по умолчанию',note:'Какая вкладка статуса выбрана при открытии',
    opts:[['all','Все'],['ok','Активные'],['low','Низкий хэшрейт'],['off','Отключены'],['fail','Оффлайн']]},
  wpage:{g:'Воркеры',label:'Строк на странице',note:'Размер страницы в таблице воркеров',opts:[['20','20'],['10','10'],['50','50']]},
  wsort:{g:'Воркеры',label:'Сортировка по умолчанию',note:'Колонка, по которой таблица открыта',
    opts:[['sh','По последней шаре'],['name','По имени'],['h24','По хэшрейту']]},
  wname:{g:'Воркеры',label:'Имена воркеров',note:'Короткие Ant01 или длинные с аккаунтом',
    opts:[['short','Короткие'],['long','Длинные']]},
  wtagn:{g:'Воркеры',label:'Теги на воркерах',note:'Сколько воркеров помечены тегами',opts:[['some','У части'],['all','У всех'],['none','Нет']]},
  wnote:{g:'Воркеры',label:'Порог уведомлений',note:'Задан ли порог уведомления в карточке',opts:[['yes','Задан'],['no','Не настроен']]},
  ser:{g:'Воркеры',label:'Серийные номера',note:'Состояние экрана серийных номеров',opts:[['no','Не заполнены'],['ok','Заполнены'],['err','Есть ошибки']]},
  upl:{g:'Воркеры',label:'Файл серийников',note:'Чем закончилась загрузка файла серийников',opts:[['no','Не выбран'],['ok','Выбран'],
  ...
  /* ---- Мои активы ---- */
  awal:{g:'Мои активы',label:'Кошельки',note:'У скольких монет заведён кошелёк',opts:[['some','Как в макете'],['all','У всех монет'],['none','Не добавлены']]},
  apay:{g:'Мои активы',label:'Автовыплаты',note:'У каких монет включены автовыплаты',opts:[['btc','Только BTC'],['all','У всех монет'],['none','Выключены']]},
  ...
  acoins:{g:'Мои активы',label:'Монеты в активах',note:'Сколько монет в таблице активов',
    opts:[['all','Четыре'],['btc','Только BTC'],['two','BTC и LTC']]},
  afee:{g:'Мои активы',label:'Комиссия сети',note:'Комиссия сети в мастере вывода',opts:[['norm','Обычная'],['high','Высокая'],['zero','Нулевая']]},
  aerr:{g:'Мои активы',label:'Ошибка поля в активах',note:'Ошибка в поле вывода или продажи',opts:[['no','Нет'],['req','Не заполнено'],
    ['low','Сумма меньше порога'],['bad','Некорректный адрес'],['code','Неверный код'],['old','Код недействителен']]},

  /* ---- Выплаты ---- */
  incn:{g:'Выплаты',label:'Записей дохода',note:'Сколько строк в истории дохода',opts:[['many','32'],['few','Три'],['none','Нет']]},
  incper:{g:'Выплаты',label:'Период дохода по умолчанию',note:'С каким периодом открывается доход',
    opts:[['day','День'],['week','Неделя'],['month','Месяц']]},
  payn:{g:'Выплаты',label:'Выплат в истории',note:'Сколько строк в истории выплат',opts:[['many','28'],['few','Одна'],['none','Нет']]},
  ...
  paytype:{g:'Выплаты',label:'Типы операций',note:'Какие операции показывает фильтр',opts:[['all','Все'],['out','Только выводы'],['sell','Только продажи']]},
  paydoc:{g:'Выплаты',label:'Документы',note:'Колонка со счётом и актом',
    opts:[['yes','Есть'],['no','Нет']]},

  /* ---- Отчет ---- */
  rdata:{g:'Отчет',label:'Данные воркеров',note:'Заполнены ли модели и заводские номера',opts:[['ok','Заполнены'],['no','Не у всех воркеров'],['del','Нет у удалённых']]},
  repn:{g:'Отчет',label:'Отчётов в списке',note:'Сколько строк в списке отчётов',opts:[['many','Шесть'],['few','Один'],['none','Нет']]},
  repst:{g:'Отчет',label:'Статус отчёта',note:'Статус верхнего отчёта в списке',opts:[['ready','Готов'],['wait','Формируется'],['err','Ошибка']]},
  repacc:{g:'Отчет',label:'Аккаунтов в отчёте',note:'Сколько аккаунтов попало в отчёт',opts:[['many','Десять'],['one','Один'],['none','Нет']]},
  repdl:{g:'Отчет',label:'Скачивание отчёта',note:'Доступна ли кнопка скачивания',opts:[['yes','Доступно'],['no','Недоступно']]},
  repw:{g:'Отчет',label:'Кошельки пула',note:'Добавлены ли кошельки пула в отчёт',opts:[['yes','Добавлены'],['no','Не добавлены']]},

  /* ---- Рефералы ---- */
  tier:{g:'Рефералы',label:'Уровень рефералки',note:'Достигнутый уровень комиссии',opts:[['0','5%'],['1','10%'],['2','15%'],['3','20%'],['4','25%']]},
  refn:{g:'Рефералы',label:'Рефералов',note:'Сколько рефералов в списке',opts:[['many','50'],['few','3'],['none','Нет']]},
  refact:{g:'Рефералы',label:'Активные рефералы',note:'Сколько рефералов из списка активны',opts:[['most','Больше половины'],['some','Единицы'],['none','Нет']]},
  refpn:{g:'Рефералы',label:'Реферальных выплат',note:'Сколько строк в реферальных выплатах',opts:[['many','28'],['few','Одна'],['none','Нет']]},
  refhash:{g:'Рефералы',label:'Хэшрейт рефералов',note:'Как далеко до следующего уровня',
    opts:[['mid','На полпути'],['near','Почти взят'],['low','Только начали']]},
  refban:{g:'Рефералы',label:'Баннер реферальной ссылки',note:'Баннер с приглашением над списком',opts:[['show','Показан'],['hide','Скрыт']]},
  reflink:{g:'Рефералы',label:'Реферальная ссылка',note:'Ссылка с меткой или без неё',opts:[['plain','Обычная'],['tag','С меткой источника']]},
  refrub:{g:'Рефералы',label:'Подсказка о рублях',note:'Уведомление над таблицей выплат',
    opts:[['show','Показана'],['hide','Скрыта']]},

  /* ---- Калькуляторы ---- */
  calchw:{g:'Калькуляторы',label:'Оборудование в расчёте',note:'Заполнены ли поля оборудования',opts:[['yes','Выбрано'],['no','Не выбрано']]},
  taxface:{g:'Калькуляторы',label:'Тип лица в налогах',note:'От типа лица зависит ставка',opts:[['pf','Физлицо'],['ip','ИП'],['ul','Юрлицо']]},
  taxres:{g:'Калькуляторы',label:'Резидентство',note:'У нерезидента ставка выше',opts:[['yes','Резидент'],['no','Нерезидент']]},

  /* ---- Аккаунт ---- */
  role:{g:'Аккаунт',label:'Роль',note:'Владелец видит всё, наблюдатель — разрешённое',opts:[['owner','Владелец'],['observer','Наблюдатель']]},
  perm:{g:'Аккаунт',label:'Разрешения вотчера',note:'Разделы, открытые в ссылке наблюдателя',opts:[['all','Все разделы'],['wi','Воркеры и доход'],
    ['wo','Только воркеры'],['fin','Только финансы'],['assets','Только мои активы'],
    ['pay','Только выплаты'],['none','Только рефералы — Главной нет']]},
  ocoins:{g:'Аккаунт',label:'Монеты наблюдателя',note:'Монеты, открытые в ссылке наблюдателя',
    opts:[['all','Все'],['btc','Только BTC'],['ltc','Только LTC + DOGE'],['zec','Только ZEC']]},
  acct:{g:'Аккаунт',label:'Аккаунт',note:'Под каким аккаунтом открыт кабинет',opts:[['main','Основной'],['sub','Суб-аккаунт']]},
  name:{g:'Аккаунт',label:'Имя аккаунта',note:'Без имени аккаунта майнинг не начать',opts:[['yes','Задано'],['no','Не задано']]},
  fa:{g:'Аккаунт',label:'Двухфакторная защита',note:'Двухфакторная защита в профиле',opts:[['no','Выключена'],['yes','Включена']]},
  fam:{g:'Аккаунт',label:'Способ второго фактора',note:'Куда приходит код второго фактора',opts:[['app','Приложение'],['sms','SMS'],['mail','Почта']]},
  sess:{g:'Аккаунт',label:'Сессии',note:'Сколько устройств в списке сессий',opts:[['many','Шесть устройств'],['few','Два устройства'],['one','Только это устройство']]},
  pwdage:{g:'Аккаунт',label:'Возраст пароля',note:'Давно ли менялся пароль',opts:[['fresh','Меняли недавно'],['old','Давно не меняли']]},
  sesgeo:{g:'Аккаунт',label:'География сессий',note:'Сессии из одного города или из разных',opts:[['mix','Разные города'],['one','Один город']]},
  del:{g:'Аккаунт',label:'Удаление аккаунта',note:'Запрошено ли удаление аккаунта',opts:[['no','Не запрошено'],['yes','Запрошено']]},

  /* ---- Профиль ---- */
  subs:{g:'Профиль',label:'Суб-аккаунты',note:'Сколько суб-аккаунтов заведено',opts:[['many','3'],['few','1'],['none','Только основной']]},
  obs:{g:'Профиль',label:'Наблюдатели',note:'Сколько ссылок наблюдателя создано',opts:[['many','6'],['few','2'],['none','Нет']]},
  notif:{g:'Профиль',label:'Уведомления',note:'Сколько уведомлений в списке',opts:[['many','12 новых'],['few','2 новых'],['none','Нет']]},
  notift:{g:'Профиль',label:'Типы уведомлений',note:'Что приходит в колокольчик',
    opts:[['all','Все'],['imp','Только важные'],['off','Выключены']]},
  sublim:{g:'Профиль',label:'Лимит суб-аккаунтов',note:'Исчерпан ли лимит суб-аккаунтов',opts:[['free','Есть запас'],['full','Исчерпан']]},
  obsexp:{g:'Профиль',label:'Ссылки наблюдателей',note:'Ссылки действуют или истекли',opts:[['ok','Действуют'],['exp','Есть истёкшие']]},
  avatar:{g:'Профиль',label:'Аватар',note:'Загружен ли аватар в профиле',opts:[['no','Не загружен'],['yes','Загружен']]},

  /* ---- Контакты ---- */
  phone:{g:'Контакты',label:'Телефон',note:'Привязан ли номер к аккаунту',opts:[['yes','Привязан'],['no','Не привязан']]},
  mail:{g:'Контакты',label:'Почта',note:'Привязана ли почта к аккаунту',opts:[['yes','Привязана'],['no','Не привязана']]},
  tg:{g:'Контакты',label:'Telegram',note:'Привязан ли Telegram к аккаунту',opts:[['no','Не привязан'],['yes','Привязан']]},
  cerr:{g:'Контакты',label:'Ошибка в поле контакта',note:'Ошибка при добавлении контакта',opts:[['no','Нет'],['req','Не заполнено'],['busy','Занято / нет бота'],['fmt','Неверный формат']]},

  /* ---- Верификация ---- */
  verif:{g:'Верификация',label:'Данные',note:'Заполнена ли анкета верификации',opts:[['no','Не заполнены'],['yes','Заполнены']]},
  vdoc:{g:'Верификация',label:'Выписка из реестра',note:'Приложена ли выписка из реестра',opts:[['no','Не добавлена'],['yes','Добавлена']]},
  vacc:{g:'Верификация',label:'Расчетный счет',note:'Добавлен ли расчётный счёт',opts:[['no','Не добавлен'],['yes','Добавлен']]},
  vform:{g:'Верификация',label:'Правовая форма',note:'Правовая форма меняет состав анкеты',opts:[['pf','Физлицо'],['ip','ИП'],['ul','Юрлицо']]},
  vstat:{g:'Верификация',label:'Статус проверки',note:'Что показывает статус проверки',
    opts:[['none','Не отправлено'],['wait','На проверке'],['ok','Принято'],['err','Отклонено']]},
  verr:{g:'Верификация',label:'Ошибка поля анкеты',note:'В каком поле анкеты ошибка',opts:[['no','Нет'],['tax','Код налоговой'],['inn','ИНН'],['bank','Банк не выбран'],['file','Файл больше 4 МБ']]},

  /* ---- Наблюдатели ---- */
  oerr:{g:'Наблюдатели',label:'Описание ссылки',note:'Ошибка в описании ссылки наблюдателя',opts:[['no','Заполнено'],['req','Не заполнено'],['long','Больше 100 символов']]},

  /* ---- Суб-аккаунты ---- */
  saerr:{g:'Суб-аккаунты',label:'Имя суб-аккаунта',note:'Проверка имени при создании суб-аккаунта',opts:[['no','Пусто'],['ok','Свободно'],
  ...
  grid:{g:'Интерфейс',label:'Сетка поверх макета',note:'Линейка для проверки отступов',
  ...
  lerr:{g:'Вход',label:'Ошибка входа',note:'Под каким полем загорается ошибка',
    opts:[['no','Нет'],['req','Поле обязательно'],['fmt','Некорректный email'],['pair','Неверный email или пароль']]},
  rerr:{g:'Вход',label:'Ошибка регистрации',note:'Под каким полем загорается ошибка',
  ...
  a2fa:{g:'Вход',label:'Второй фактор при входе',note:'Что просят после пароля',
    opts:[['no','Нет'],['code','Код на почту'],['ga','Google Authenticator']]},
  otperr:{g:'Вход',label:'Код подтверждения',note:'Поле кода на экране подтверждения',
    opts:[['ok','Введён'],['no','Пусто'],['bad','Введен неверный код']]},

  /* ---- Загрузка и ошибки ---- */
  load:{g:'Загрузка и ошибки',label:'Загрузка',note:'Скелетон или ошибка вместо содержимого',opts:[['no','Загружено'],['yes','Скелетон'],['err','Ошибка загрузки']]},
...
const PRESETS=[
  ...
  ['Без анимаций','Для записи видео и скриншотов',{motion:'off',toast:'off'},'Интерфейс'],
  ['Разметка','Сетка 8 и границы блоков поверх экрана',{grid:'px8',outline:'on'},'Интерфейс'],
  ...
  ['Лимиты исчерпаны','Некуда расти: суб-аккаунты и ссылки кончились',
    {sublim:'full',obsexp:'exp',subs:'many',obs:'many'},'Доступ'],
  ['Большой парк','Четыреста воркеров и длинные имена',
    {wn:'400',wname:'long',wpage:'50',wsort:'name',wgeo:'many'},'Жизненный цикл'],
  ['Парк на грани','Низкий аптайм, старые шары, высокий реджект',
    {health:'critical',wup:'low',wshare:'old',rej:'high',trend:'down',gap:'many'},'Проблемы'],
];
const DEF={
  coin:'btc',data:'normal',health:'degraded',wf:'yes',prec:'full',fiat:'both',numfmt:'ru',tzview:'local',
  ...
  wfilt:'all',wpage:'20',wsort:'sh',wname:'short',wtagn:'some',
  awal:'some',apay:'btc',athr:'def',asell:'yes',astep:'3',aconf:'mail',aerr:'no',acoins:'all',afee:'norm',
  payn:'many',payst:'mix',paytype:'all',paydoc:'yes',incn:'many',incper:'day',
  rdata:'ok',repn:'many',repst:'ready',repw:'yes',repacc:'many',repdl:'yes',
  tier:'0',refn:'many',refact:'most',refpn:'many',refrub:'show',refhash:'mid',refban:'show',reflink:'plain',
  calchw:'yes',taxface:'pf',taxres:'yes',
  role:'owner',perm:'all',ocoins:'all',acct:'main',name:'yes',fa:'no',fam:'app',sess:'many',del:'no',
  pwdage:'fresh',sesgeo:'mix',
  subs:'many',obs:'many',notif:'many',notift:'all',avatar:'no',sublim:'free',obsexp:'ok',
...
const dec=(v,max=8)=>{
  if(!v) return '0';
  if(S.numfmt==='plain') return String(Number(Number(v).toFixed(max)));
  return Number(v).toLocaleString(LOC(),{minimumFractionDigits:0,maximumFractionDigits:max});
};
/* Ось «Формат чисел»: русские разряды, английские или вовсе без них */
const LOC=()=>S.numfmt==='en'?'en-US':'ru-RU';
const nf=(v,d=2)=>S.numfmt==='plain'
  ? Number(v).toFixed(d)
  : Number(v).toLocaleString(LOC(),{minimumFractionDigits:d,maximumFractionDigits:d});
const ni=v=>S.numfmt==='plain'?String(Math.round(Number(v))):Number(v).toLocaleString(LOC());
...
const shareAt=off=>{const d=new Date(SHARE_BASE.getTime()-off*60000-(S.tzview==='utc'?3*3600000:0));
  return `${d.getDate()} ${MONTHS_R[d.getMonth()]}, ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}${S.tzview==='utc'?' UTC':''}`};
/* Пока фильтр статуса не трогали руками, его задаёт ось «Фильтр по умолчанию» */
const wfilterOf=()=>U.wfilter??S.wfilt;
...
function workersList(m){
  ...
  for(let i=0;i<N;i++){
    ...
    /* Ось «Имена воркеров»: короткое Ant01 или длинное с аккаунтом */
    const nm=S.wname==='long'?'pmineraccount.00'+(101+i)+'.Ant'+String(i+1).padStart(2,'0')
      :'Ant'+String(i+1).padStart(2,'0');
    out.push(Object.assign({id:i+1,name:nm,model:models[i%4],st,
      h5:base+5,h1:base+12,h24:base+8,rej:((i%8)+1)/100*rejK,up:upB+(i%2),
      /* Ось «Теги на воркерах»: у части, у всех или ни у кого */
      tag:S.wtagn==='none'?'':tg[0],
      tags:S.wtagn==='none'?[]:S.wtagn==='all'?tg:(extra?tg:[tg[0]]),
      extra:S.wtagn==='none'?0:S.wtagn==='all'?1:extra,
...
function workersRows(m){
  ...
  /* Ось «Фильтр по умолчанию» задаёт вкладку статуса, пока её не трогали руками */
  const wf=wfilterOf();
  if(wf!=='all') rows=rows.filter(w=>w.st===wf);
  ...
  /* Ось «Сортировка по умолчанию»: свежая шара, имя или хэшрейт */
  else if(S.wsort==='name') rows=[...rows].sort((a,b)=>a.name.localeCompare(b.name,'ru'));
  else if(S.wsort==='h24') rows=[...rows].sort((a,b)=>b.h24-a.h24);
...
V.workers=m=>{
  ...
  const cnt=k=>PC[k];
  const per=perOf('workers',+S.wpage||20), pages=Math.max(1,Math.ceil(shown.length/per));
  ...
  const rows=page.map(w=>{const[lbl,col]=WST[w.st];
    return `<tr data-wk="${w.id}" class="${U.sel.has(w.id)?'sel':''}">
      ...
      <td class="tags">${tagsOf().length&&w.tag
  ...
  ${card(`<div class="ch subhead">
    <div class="seg">
      ${[['all','Все',''],['ok','Активные','var(--pos)'],['low','Низкий хэшрейт','var(--warn)'],['off','Отключены','var(--neg)'],['fail','Оффлайн','var(--neu)']]
        .map(([k,l,c])=>`<button class="${wfilterOf()===k?'on':''}" data-wf="${k}">${c?`<i class="dot" style="background:${c}"></i>`:''}<span class="sl">${l}</span>${k==='low'?'<span class="ss">Низкий хэш</span>':''} <u>${ni(cnt(k))}</u></button>`).join('')}
    </div>
    ${all.length?`
  ...
  ${netFail()?tblFail():all.length?(shown.length?`<div class="tw"><table class="tbl wtbl"><thead><tr>
      ...
      ${pager('workers',shown.length,+S.wpage||20)}`
...
V.serials=m=>{
  ...
  const per=perOf('serials',+S.wpage||20), pages=Math.max(1,Math.ceil(rows.length/per));
  ...
  ${page.length?`<div class="tw"><table class="tbl sertbl"><thead><tr>
    ...
    ${pager('serials',rows.length,+S.wpage||20)}`
...
/* Ось «Монеты в активах»: сколько строк в таблице «Мои активы» */
const assetsOf=()=>S.acoins==='btc'?ASSETS.slice(0,1):S.acoins==='two'?ASSETS.slice(0,2):ASSETS;
/* Ось «Комиссия сети» — довесок к сумме в мастере вывода */
const feeOf=a=>S.afee==='zero'?'0':S.afee==='high'?'0,0004 '+a.s:'0,00002 '+a.s;
...
V.assets=m=>{
  ...
  const sum=f=>nf(assetsOf().reduce((s,a)=>s+a[f],0)*k,0);
...
const csel=(id,cur,off)=>`<span class="pop-wrap"><button class="csel ${pop===id?'open':''}" data-pop="${id}">${COIN_ICON[cur]}${cur}<span class="spacer"></span>${I.cd}</button>
  ${pop===id?`<div class="pop menu xmenu csmenu">${assetsOf().map((a,n)=>{
    const d=off&&off(a);
...
const periodN=(segId,dateKey,base,def)=>{
  const i=U.seg[segId]??def;
  let n=(i===undefined||i===null)?base:Math.max(1,Math.round(base*PERIOD_K[i]));
...
/* Ось «Записей дохода» — длина истории начислений */
const incN=m=>m.empty?0:S.incn==='none'?0:S.incn==='few'?3:48;
/* Ось «Период дохода по умолчанию» — какой чип периода горит при открытии */
const incPer=()=>({day:0,week:1,month:2}[S.incper]??0);
...
${card(`<div class="ch"><h2>График дохода (${m.bal[0].s})</h2><div class="spacer"></div>
  ${dayChips('income-chart',incPer())}${dateInput('inc1')}
  ${zoomBtns()}</div>
...
${card(`<div class="ch"><h2>История дохода</h2><div class="spacer"></div>
  ${dayChips('income-hist',incPer())}${dateInput('inc2')}
  <button class="ib" data-modal="export" data-ex="inc">${I.dl}</button></div>
  ${incomeTable(m,periodN('income-hist','inc2',incN(m),incPer()),'income')}`)}`;
...
/* Ось «Аккаунтов в отчёте»: длинный список, один аккаунт или пусто */
const REP_ACCS='natarusso, ivanivanov, loricarson, natapetrova, alex12345, petrminer77, sergeiv, billterry, petrovan, alexminer';
const repAcc=a=>S.repacc==='none'?'':S.repacc==='one'?'natarusso':(a?REP_ACCS:a);
...
V.report=m=>{
  ...
  ${repRows().map(([mo,y,a,d,st])=>{const [cls,lab]=REP_ST[st];
    ...
    return `<tr><td>${mo}</td><td class="mono">${y}</td>
      <td class="racc">${m.empty||!repAcc(a)?'':`<span data-tip="${repAcc(a)}">${repAcc(a)}</span>`}</td>
      <td class="mono mut">${m.empty?'':d}</td>
      <td>${status(cls,lab,'caps')}</td>
      <td class="num">${noV&&!ro?`<span data-tip="${REP_TIP}">${gen}</span>`:gen}</td>
      <td class="num"><button class="btn out xs" ${st==='ok'&&S.repdl!=='no'?'':'disabled'} data-toast="Отчет скачан">${I.dl}Скачать</button></td></tr>`}).join('')}
...
V.sumassets=m=>{
  const k=m.empty?0:1;
  const sum=f=>nf(assetsOf().reduce((a,x)=>a+x[f],0)*k,0);
  ...
  ${card(`<div class="tw"><table class="tbl atbl"><thead><tr><th>Монеты</th><th>Баланс</th>
    ...
    ${assetsOf().map(a=>`<tr><td><span class="coin">${COIN_ICON[a.s]}${a.s}</span></td>
...
${card(`<div class="ch"><h2>Настройка реферальных выплат</h2></div>
  ...
  ${assetsOf().map(a=>`<tr>
    ...
    <td><button class="acell" data-modal="athr" data-coin="${a.s}">${thrOf(a)}<i>${I.edit}</i></button></td>
...
/* Ось «Хэшрейт рефералов» двигает заполнение дорожки и остаток до уровня */
const REF_FILL={low:.35,mid:1,near:1.6};
const refFill=()=>Math.max(0,Math.min(100,TIERS[+S.tier].f*(REF_FILL[S.refhash]??1)));
const refNext=m=>S.refhash==='near'?'8 200 '+m.c.unit:S.refhash==='low'?'182 400 '+m.c.unit:m.c.next;
/* Ось «Реферальная ссылка»: с меткой источника или без */
const refUrl=()=>LINKS.ref(49282838)+(S.reflink==='tag'?'?utm_source=telegram':'');
...
const obsOf=m=>{
  ...
  /* Ось «Ссылки наблюдателей»: каждая третья ссылка просрочена */
  const all=OBSERVERS.slice(0,OBS_N[S.obs]??6)
    .map((o,i)=>S.obsexp==='exp'&&i%3===1?{...o,expired:1,term:'до 12.01.2026'}:o);
...
/* Ось «География сессий»: все входы из одного города или из разных */
const sessOf=m=>SESSIONS.slice(0,m.empty?1:(SESS_N[S.sess]??6))
  .map(x=>S.sesgeo==='one'?{...x,loc:'Москва, Россия'}:x);
...
/* Ось «Правовая форма»: какой чип выбран при открытии анкеты */
const vformI=()=>({ip:0,ul:1,pf:2}[S.vform]??2);
/* Ось «Статус проверки»: плашка над анкетой */
const VSTAT={none:'',
  wait:`<div class="alert info" style="margin:0 0 16px">Анкета на проверке — обычно это занимает один рабочий день</div>`,
  ok:`<div class="alert ok" style="margin:0 0 16px">Данные проверены и приняты</div>`,
  err:`<div class="alert err" style="margin:0 0 16px">Данные отклонены: проверьте ИНН и код налоговой</div>`};
...
V.verification=m=>{
  /* Ось «Правовая форма» задаёт выбранный чип, пока его не трогали руками */
  const form=VFORMS[segi('vform',vformI())][0];
  const full=form!=='fiz';
  ...
  <div class="vcol">
    ${card(`<div class="vform">
      ...
      <div class="vlist">${VFORMS.map(([,label],i)=>
        `<button class="chip ${i===segi('vform',vformI())?'on':''}" data-seg="vform" data-i="${i}" ${RO()}>${label}</button>`).join('')}</div>`)}
    <div class="vnote">${VHINTS.map(([t,list])=>`<div class="vnb"><b>${t}</b>
      <ul>${list.map(x=>`<li>${x}</li>`).join('')}</ul></div>`).join('')}</div>
  </div>
  <div class="vcol">
    ${card(`<h2 class="vh2">Верификация</h2>${VSTAT[S.vstat]||''}
...
V.subaccounts=m=>{
  ...
  ${limit?`<span data-tip="Достигнут лимит суб-аккаунтов для вашего тарифа"><button class="btn" disabled>${I.pl} Добавить суб-аккаунт</button></span>`
...
V.security=m=>{
  ...
  <div style="display:flex;flex-direction:column;gap:var(--sp4)">
    ...
    ${card(`<h2>Безопасность данных</h2>
      ${secRow(I.key,'Ваш пароль',S.pwdage==='old'
        ? 'Пароль не меняли больше года — самое время его обновить'
        : 'Этот пароль используется для входа в ваш аккаунт',
    ...
    b:(m,step)=>{const n=wSteps(), a=curAsset(),e=aerrBox(),w=walletOf(a)||'Не добавлен';
      ...
      if(n===4&&step===1) return `<div class="mstack">${prog(1,n)}
        <div class="acont">
          ...
          <div class="amt"><span class="k">Комиссия сети</span><b class="mono">${feeOf(a)}</b></div>
        </div>
